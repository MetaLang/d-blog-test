...
author: DBlogAdmin
comments: false
date: 2017-03-20 14:03:25+00:00
layout: post
link: https://dlang.org/blog/2017/03/20/dont-fear-the-reaper/
slug: dont-fear-the-reaper
...
[Closures](https://dlang.org/spec/function.html#closures), which are delegates or function literals that need to carry around a pointer to the local stack frame, may also allocate. The last allocating language feature listed on [the Garbage Collection page](http://dlang.org/spec/garbage.html) is the [assertion](http://dlang.org/spec/expression.html#AssertExpression). An assertion will allocate when it fails because it needs to throw an `AssertError`, which is part of D's class-based exception hierarchy (we'll look at how classes interact with the GC in a future post).

Then there's [Phobos](https://dlang.org/phobos/index.html), D's standard library. Once upon a time, much of Phobos was implemented with little concern for GC allocations, making it difficult to use in situations where they were undesirable. However, a massive effort was initiated
to make it more conservative in its GC usage. Some functions were made to work with lazy ranges, others were rewritten to take buffers supplied by the caller, and some were reimplemented to avoid unnecessary allocations internally. The result is a standard library more amenable to GC-free code (though there are still probably some corners of the library that have not yet been renovated -- [PRs welcome](https://github.com/dlang/phobos)).

Now that we've seen the basics of using the GC, [the next post](http://dlang.org/blog/2017/06/16/life-in-the-fast-lane/) in [this series](http://dlang.org/blog/the-gc-series/) looks at the tools the language and the compiler provide for turning the GC off and making sure specific sections of code are GC-free.
